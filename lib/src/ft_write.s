...
ft_write:
	...
	jl .ft_write.error	; If the result of the previous comparison is negative (== if value in RAX is less than 0), jump to the code section with label 'error'. This is becaus we will need to set the errno.
	ret

	.ft_write.error:
		mov	rdx, rax	; We are going to call a function so will need to copy what was in RAX. We are storing it into RDX.
